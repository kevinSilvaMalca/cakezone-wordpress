				...
				if ($enable_sortable == 'yes'):
					...
					if ($taxonomies) {
						...
						while ($projects->have_posts()) {
							...
							$current_prtfl_terms = get_the_terms(get_the_ID(), 'lafka_foodmenu_category');
							if(is_array($current_prtfl_terms))  {
							    $foodmenu_categories = array_merge($foodmenu_categories, $current_prtfl_terms);
							}
						}

						$projects->rewind_posts();
						foreach ($foodmenu_categories as $cat) {
							$foodmenu_categories_unique[$cat->term_id] = $cat;
						}
					}
					?>
					<div class="lafka-foodmenu-categories">
						<ul>
							...
							<?php foreach ($foodmenu_categories_unique as $category): ?>
...
if (!function_exists('lafka_map_shortcode')) {
    ...
    function lafka_map_shortcode($atts) {
        ...
        if ($map_latitude && $map_longitude && !is_search()) {
            ...
            // Map config
            wp_enqueue_script('lafka-plugin-map-config-' . $map_canvas_unique_id, plugins_url("assets/js/lafka-plugin-map-config.min.js", dirname(__FILE__)), array('google-maps'), false, true);
            wp_localize_script('lafka-plugin-map-config-' . $map_canvas_unique_id, 'lafka_map_short', array(
                    'location_title' => esc_js($location_title ? $location_title : esc_html__('Our Location', 'lafka-plugin')),
...
if (!function_exists('lafka_woo_top_rated_carousel_shortcode')) {
    ...
    function lafka_woo_top_rated_carousel_shortcode($atts) {
        ...
        return $js_config_output . '<div id="' . esc_attr($unique_id) . '">' . $shortcode->get_content() . '</div>';
    }
...
if (!function_exists('lafka_woo_recent_viewed_products_shortcode')) {
    ...
    function lafka_woo_recent_viewed_products_shortcode($atts) {
        ...
        return $js_config_output . $products_output;
    }
...
if (!function_exists('lafka_woo_products_slider_shortcode')) {
    ...
    function lafka_woo_products_slider_shortcode($atts) {
        ...
        return $js_config_output . '<div id="' . esc_attr($unique_id) . '" class="lafka-product-slider owl-carousel">' . ob_get_clean() . '</div>';
    }
