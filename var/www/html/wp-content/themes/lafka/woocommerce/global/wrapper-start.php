...
<div id="content" class="content-area <?php echo esc_attr(implode(' ', $sidebar_classes)) ?>">
    ...
    <?php if (!is_product()): // For single product don't show title div ?>
        <?php if (is_shop()): // For SHOP page ?>
            <div id="lafka_page_title" class="lafka_title_holder <?php echo esc_attr(lafka_get_option('shop_title_alignment')) ?> <?php if ($title_background_image): ?>title_has_image<?php endif; ?>">
   		...
   		<?php elseif(is_product_category() || is_product_tag()):  // For Category page ?>
            <div id="lafka_page_title" class="<?php echo implode(" ", array("lafka_title_holder", esc_attr($lafka_prod_category_header_alignment))) ?><?php if ($lafka_prod_category_header_img_id): ?> title_has_image<?php endif; ?>">
