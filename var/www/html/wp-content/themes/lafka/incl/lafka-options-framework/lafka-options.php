...
function lafka_optionsframework_options() {
	...
	$options[] = array(
		...
		'desc' => sprintf( wp_kses( _x( 'Enter your Google Maps JavaScript API key, to be used for google map integration in Map shortcode. <a target="_blank" href="%s">Generate Google Maps JavaScript API key</a>', 'theme-options', 'lafka' ), array(
			'a' => array(
				'target' => array(),
				'href'   => array()
			)
	...
	$options[] = array(
			...
			'desc' => esc_html_x('Enter Copyright text.', 'theme-options', 'lafka'),
			'id' => 'copyright_text',
			'std' => 'Lafka theme by <a target="_blank" title="theAlThemist\'s foodmenu" href="http://themeforest.net/user/theAlThemist/foodmenu?ref=theAlThemist">theAlThemist</a> | &#169; 2019 All rights reserved!',
			'type' => 'textarea'
	);
	...
	if (defined('LAFKA_IS_WOOCOMMERCE') && LAFKA_IS_WOOCOMMERCE) {
		...
		$options[] = array(
			...
			'desc' => esc_html_x( 'With Order Hours enabled, you can control when the store is open or closed for online orders. There are variety of options: Time zone, Weekly schedule, Instant Open / Close functionality, Holiday period. Once enabled, all settings are available under WooCommerce -> Lafka Order Hours. ', 'theme-options', 'lafka' ) .
			          '<br><br><b>' . esc_html_x( 'IMPORTANT NOTE', 'theme-options', 'lafka' ) . ': </b>' .
		...
		$options[] = array(
			'name'    => esc_html_x( 'Manage Buttons Visibility in Product Listings', 'theme-options', 'lafka' ),
		...
		$options[] = array(
			'name'    => esc_html_x( 'Hover Product Image Behaviour in Product Listings', 'theme-options', 'lafka' ),
