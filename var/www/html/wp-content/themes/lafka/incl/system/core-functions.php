...
if (!function_exists('lafka_register_required_plugins')) {
	...
	function lafka_register_required_plugins() {
		...
		$plugins = array(
				array(
						'name' => esc_html__('Lafka Plugin - Lafka Theme accompanying plugin ', 'lafka'), // The plugin name
						'slug' => 'lafka-plugin', // The plugin slug (typically the folder name)
						'source' => get_template_directory() . '/plugins/lafka-plugin.zip', // The plugin source
                ...
                array(
                        'name' => esc_html__('Envato Market - WordPress Theme & Plugin management for the Envato Market', 'lafka'), // The plugin name
				...
				array(
						'name' => esc_html__('WooCommerce - excelling eCommerce', 'lafka'), // The plugin name
				...
				array(
						'name' => esc_html__('YITH WooCommerce Wishlist', 'lafka'), // The plugin name
				...
				array(
						'name' => esc_html__('Revolution Slider', 'lafka'), // The plugin name
				...
				array(
						'name' => esc_html__('WPBakery Page Builder', 'lafka'), // The plugin name
...
if (!function_exists('lafka_enqueue_admin_js')) {
	...
	function lafka_enqueue_admin_js($hook) {
		...
		wp_enqueue_script('lafka-back', get_template_directory_uri() . "/js/lafka-back.js", array('jquery', 'nice-select', 'wp-color-picker'), false, true);
...
if (!function_exists('lafka_typography_google_fonts_url')) {
	...
	function lafka_typography_google_fonts_url() {
		...
		if (!empty($font_families)) {
			...
			$font_url = add_query_arg('family', urlencode($font_families_string_to_encode . ':100,100italic,200,200italic,300,300italic,400,400italic,500,500italic,600,600italic,700,700italic,800,800italic,900,900italic&subset=' . lafka_get_google_subsets()), "//fonts.googleapis.com/css");
		}
...
function lafka_typography_enqueue_google_font() {
	wp_enqueue_style('lafka-fonts', lafka_typography_google_fonts_url(), array(), '1.0.0');
}
...
if (!function_exists('lafka_enqueue_scripts_and_styles')) {
	...
	function lafka_enqueue_scripts_and_styles() {
		...
		wp_enqueue_style('lafka-style', get_stylesheet_uri());
		...
		// Mega Menu script/style
		wp_register_style('lafka-mega-menu', get_template_directory_uri() . '/styles/lafka-admin-megamenu.css');
		wp_register_script('lafka-mega-menu', get_template_directory_uri() . '/js/lafka-admin-mega-menu.js', array('jquery', 'jquery-ui-sortable'), false, true);
		...
		wp_register_script('google-maps', 'https://maps.googleapis.com/maps/api/js?'.( lafka_get_option('google_maps_api_key') ? 'key='.lafka_get_option('google_maps_api_key').'&' : '' ).'sensor=false', array('jquery'), false, true);
		...
		/* Include js configs */
		wp_enqueue_script('lafka-libs-config', get_template_directory_uri() . "/js/lafka-libs-config.min.js", array('jquery', 'wp-util', 'flexslider', 'owl-carousel', 'cloud-zoom', 'countdown', 'magnific', 'appear', 'typed', 'nice-select', 'is-in-viewport' ), false, true);
...
add_action('init', 'lafka_mega_menu_init');
if (!function_exists('lafka_mega_menu_init')) {

	// Initialise LafkaMegaMenu class
	function lafka_mega_menu_init() {
		$init_mega_menu = new LafkaMegaMenu();
	}
...
if (!function_exists('lafka_check_for_sidebar')) {
	...
	function lafka_check_for_sidebar() {
		...
		if (LAFKA_IS_WOOCOMMERCE && is_woocommerce() && isset($woocommerce_sidebar)) {
